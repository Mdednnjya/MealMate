...
export default function DonationDetailPage({ params }: { params: { id: string } }) {
    ...
    const handleStatusChange = async (requestId: string, newStatus: string, requestQuantity: number) => {
        ...
        if (donation) {
            const newQuantity = donation.quantity - requestQuantity;
            const newStatus = newQuantity === 0 ? 'COMPLETED' : 'AVAILABLE';
        ...
        const { data: requestData, error: requestError } = await supabase
            .from('requests')
            .select('user_id, donations!inner(id, title, user_id)')
            .eq('id', requestId)
            .single();
        ...
        if (requestData) {
            const typedRequestData = requestData as unknown as RequestData;

            if (typedRequestData.donations && typedRequestData.donations.length > 0) {
                const donation = typedRequestData.donations[0];
                const notificationType = newStatus === 'ACCEPTED' ? 'REQUEST_ACCEPTED' : 'REQUEST_REJECTED';

                await supabase
                    .from('notifications')
                    .insert({
                        user_id: typedRequestData.user_id,
                        sender_id: donation.user_id,
            ...
            {donation.requests.map((request) => (
                <div key={request.id} className="bg-white rounded-lg shadow-lg px-6 py-4 sm:p-10 mb-6">
                    ...
                    <div className="mt-6 flex flex-col sm:flex-row justify-center space-y-2 sm:space-y-0 sm:space-x-2">
                        <button
                            onClick={() => handleStatusChange(request.id, 'ACCEPTED', request.quantity)}
                            className="bg-black text-white px-4 sm:px-14 py-2 rounded-lg w-full sm:w-auto"
                        >
                            Accept
                        </button>
                        <button
                            onClick={() => handleStatusChange(request.id, 'REJECTED', 0)}
